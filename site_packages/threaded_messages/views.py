...
@login_required
def inbox(request, template_name='django_messages/inbox.html', base='base.html'):
    """
...
@login_required
def search(request, template_name="django_messages/search.html", base='base.html'):
...
@login_required
def outbox(request, template_name='django_messages/inbox.html', base='base.html'):
    """
...
@login_required
def trash(request, template_name='django_messages/trash.html', base='base.html'):
    """
...
def compose(request, recipient=None, form_class=ComposeForm,
        template_name='django_messages/compose.html', success_url=None, recipient_filter=None, base='base.html'):
    """
    ...
    if request.method == "POST":
        sender = request.user
        getSender = User.objects.get(pk=sender.id)
        ...
        if form.is_valid():
            ...
            recipients = form.data['recipient'].rstrip(',').split(',')
            message = form.data['body']
            for recipient in recipients:
                getRecipient = User.objects.get(username=recipient)
                getRecipientProfile = UserProfile.objects.get(user=getRecipient)
                if getRecipientProfile.receive_notifications == 1 and getRecipient != request.user:
                    html_content = render_to_string('email/new_message.html', {'sender':getSender,'recipient':getRecipient,'recipientProfile':getRecipientProfile,'message':message}) # ...
                    text_content = strip_tags(html_content) # this strips the html, so people will have the text as well.

                    # create the email, and attach the HTML version as well.
                    msg = EmailMultiAlternatives('New Message', text_content, 'Lionsmatter@lionsmatter.com', [getRecipientProfile.notification_email])
...
def view(request, thread_id, form_class=ReplyForm,
        success_url=None, recipient_filter=None, template_name='django_messages/view.html', base='base.html'):
    """
...
def message_ajax_reply(request, thread_id,
                  template_name="django_messages/message_list_view.html", base='base.html'):
    if request.META['HTTP_HOST'] == 'tail.lionsmatter.com':
        base = 'tail/base.html'
    thread = get_object_or_404(Thread, id=thread_id)
    if request.POST:
    ...
    if request.GET.get("format") == "json":
        ...
        for user in users:
            data.append({"id": user.username,
                         "url": reverse("userprofile",args=(user.username,)),
                         "name": "%s %s"%(user.username, user.last_name)})
